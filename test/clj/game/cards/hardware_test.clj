...
(deftest flip-switch-trace-reaction-ability
    ...
    (do-game
      ...
      (take-credits state :runner)
      (play-from-hand state :corp "SEA Source")
      (is (waiting? state :runner) "Runner shouldn't get to use Flip Switch on Corp's turn")
      ...
      (is (waiting? state :corp) "Corp should now be waiting on Runner for Flip Switch")
      (click-prompt state :runner "Yes")
...
(deftest flip-switch-jack-out-ability
    ...
    (do-game
      ...
      (let [flip (get-hardware state 0)]
        (run-on state "HQ")
        (card-ability state :runner (get-hardware state 0) 0)
        (is (= "Test Runner jacks out." (-> @state :log last :public :text)))
...
(deftest masterwork-v37
  ...
  (do-game
      ...
      (run-on state "HQ")
      (is (waiting? state :corp) "Corp should be waiting on Runner")
      (let [credits (:credit (get-runner))]
        (click-prompt state :runner "Yes")
